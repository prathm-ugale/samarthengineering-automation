...
@Component({
  ...
  template: `
    <div class="sticky-actions">
      ...
      <!-- Quick RFQ: Circular Matching WhatsApp, Expands on Hover -->
      <button (click)="openQuote()" class="sticky-btn rfq-btn" title="Request Instant RFQ" aria-label="Request Instant RFQ">
        <span class="rfq-icon-box">
        ...
        <span class="rfq-text">Quick RFQ</span>
      </button>

      <!-- Scroll to Top Button -->
      <button (click)="scrollToTop()" class="sticky-btn top-btn" title="Scroll to top" aria-label="Scroll to top of page">
  ...
  styles: [`
    ...
    /* WhatsApp Button: Icon-Only, Circular */
    .whatsapp-btn {
      width: 48px;
      height: 48px;
      padding: 0;
    ...
    /* Quick RFQ Button: Exact same circular size, expands on hover */
    .rfq-btn {
      width: 48px;
      height: 48px;
      padding: 0;
    ...
    .rfq-text {
      ...
      transition: max-width 0.35s cubic-bezier(0.34, 1.3, 0.64, 1), 
                  opacity 0.22s ease, 
                  padding 0.3s ease;
      padding-right: 0;
      pointer-events: none;
    ...
    .rfq-btn:focus-visible {
      ...
      box-shadow: 0 10px 28px rgba(240, 120, 34, 0.48);
      background: linear-gradient(135deg, #ffa82e 0%, #f07822 100%);
      color: #ffffff;
    }
    .rfq-btn:hover .rfq-text,
    .rfq-btn:focus-visible .rfq-text {
      max-width: 100px;
      opacity: 1;
      padding-right: 16px;
    }
    ...
    .top-btn {
      ...
      box-shadow: 0 4px 14px rgba(11, 30, 51, 0.22);
    ...
    .top-btn:hover {
      ...
      box-shadow: 0 8px 20px rgba(0, 124, 122, 0.35);
    }
    ...
    @media (max-width: 768px) {
      ...
      /* On mobile touch devices, tap triggers hover expansion or can stay compact */
      .rfq-btn:hover,
      .rfq-btn:active {
        width: 136px;
      }
...
export class StickyActionsComponent {
  ...
  // Open the global quotation modal when Quick RFQ is clicked
  openQuote(): void {
    this.quoteService.open({ source: 'Sticky Actions RFQ Button' });
  }

  // Smoothly scroll the browser viewport back to the top of the page
  scrollToTop(): void {
